...
import { getPlaylistDetail, addPlaylistComment, likeComment, deleteComment, addSongToPlaylist, removeSongFromPlaylist, getAllSongs } from '@/api/system'
...
const removeSong = async (row: Song, e: Event) => {
  e.stopPropagation()
  try {
    const res = await removeSongFromPlaylist(playlistId.value, row.songId)
    if (res.code === 0) {
...
const fetchAvailableSongs = async () => {
  ...
  try {
    const res = await getAllSongs({
      ...
      songName: songSearchKeyword.value || undefined,
    })
    if (res.code === 0 && res.data) {
      const data = res.data as any
      const items = (data.items || data.records || []) as any[]
      availableSongs.value = items.map((item: any) => ({
        songId: item.songId,
        songName: item.songName || item.name,
        artistName: item.artistName,
        album: item.album,
        duration: item.duration,
        coverUrl: item.coverUrl || item.cover_url || coverImg,
        audioUrl: item.audioUrl || item.audio_url,
        likeStatus: item.likeStatus || 0,
        releaseTime: item.releaseTime || item.release_time,
      }))
...
const currentPlaylistSongIds = computed(() => new Set(songs.value.map(s => s.songId)))
...
const confirmAddSongs = async () => {
  ...
  for (const songId of selectedSongIds.value) {
    try {
      const res = await addSongToPlaylist(playlistId.value, songId)
      if (res.code === 0) {
        successCount++
      } else {
        failCount++
  ...
  if (successCount > 0) {
    ElMessage.success(`成功添加 ${successCount} 首歌曲${failCount > 0 ? `，${failCount} 首失败` : ''}`)
...
const refreshPlaylistDetail = async () => {
  ...
  if (res.code === 0 && res.data && typeof res.data === 'object' && 'songs' in res.data) {
    ...
    playlistStore.setPlaylistInfo({
      ...
      creator: { nickname: 'Vibe Music', avatarUrl: coverImg },
...
<template>
  <div class="flex flex-col h-full bg-background flex-1 md:overflow-hidden">
    <div class="flex flex-col md:flex-row p-6 gap-6">
      ...
      <div class="flex flex-col justify-between">
        ...
        <div class="flex items-center gap-4 mt-4">
          ...
          <button @click="openAddSongDialog"
            class="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input bg-hoverMenuBg h-10 rounded-lg border-2 border-gray-300 px-4">
            <icon-ic:baseline-playlist-add class="text-xl" />
            添加歌曲</button>
        </div>
      </div>
    </div>
        ...
        <div v-show="activeTab === 'songs'">
          <Table :data="songs">
            ...
            <template #actions="{ row }">
              <el-button text circle @click.stop="removeSong(row, $event)" class="opacity-0 group-hover:opacity-100 transition-opacity">
                <icon-material-symbols:delete-outline class="text-lg text-red-400 hover:text-red-600" />
              </el-button>
            </template>
          </Table>
        </div>
    ...
    <el-dialog v-model="showAddSongDialog" title="添加歌曲到歌单" width="700px" :close-on-click-modal="false">
      <div class="mb-4">
        <el-input v-model="songSearchKeyword" placeholder="搜索歌曲..." clearable @keyup.enter="handleSongSearch" @clear="handleSongSearch">
          <template #append>
            <el-button @click="handleSongSearch">搜索</el-button>
          </template>
        </el-input>
      </div>
      <div class="max-h-96 overflow-y-auto">
        <div v-if="songSearchLoading" class="text-center py-8 text-gray-400">加载中...</div>
        <div v-else-if="availableSongs.length === 0" class="text-center py-8 text-gray-400">没有找到歌曲</div>
        <div v-else v-for="song in availableSongs" :key="song.songId"
          class="flex items-center gap-3 p-3 rounded-lg hover:bg-hoverMenuBg cursor-pointer transition-colors"
          :class="{ 'opacity-50': currentPlaylistSongIds.has(song.songId) }"
          @click="currentPlaylistSongIds.has(song.songId) ? null : toggleSongSelection(song.songId)">
          <el-checkbox :model-value="isSongSelected(song.songId)" :disabled="currentPlaylistSongIds.has(song.songId)" @click.stop />
          <img :src="song.coverUrl || coverImg" class="w-10 h-10 rounded-md object-cover" />
          <div class="flex-1 min-w-0">
            <div class="text-sm font-medium truncate">{{ song.songName }}</div>
            <div class="text-xs text-gray-400 truncate">{{ song.artistName }}</div>
          </div>
          <div v-if="currentPlaylistSongIds.has(song.songId)" class="text-xs text-gray-400 shrink-0">已在歌单中</div>
          <div v-else class="text-xs text-gray-400 shrink-0">{{ song.album }}</div>
        </div>
      </div>
      <div v-if="songTotal > songPageNum * songPageSize" class="text-center mt-4">
        <el-button text @click="handleLoadMore">加载更多</el-button>
      </div>
      <template #footer>
        <el-button @click="showAddSongDialog = false">取消</el-button>
        <el-button type="primary" :disabled="selectedSongIds.size === 0" @click="confirmAddSongs">
