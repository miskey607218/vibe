...
public class AlbumDetailViewModel extends ViewModel {
    ...
    public void loadSongs(Context context, String albumName) {
        MyApplication app = (MyApplication) context.getApplicationContext();
        if (app.isLoggedIn()) {
            loadFromServer(app, albumName);
        } else {
            loadFromLocal(context, albumName);
        }
    }

    private void loadFromServer(MyApplication app, String albumName) {
                            ...
                            for (int i = 0; i < items.length(); i++) {
                                ...
                                if (item != null) {
                                    ...
                                    try {
                                        ...
                                        songs.add(new Song(songId, name, artist, albumName, formattedDuration, cover, streamAudio));
                                    } catch (NumberFormatException e) {
                                        songs.add(new Song(songId, name, artist, albumName, dur, cover, streamAudio));
                                    }
                                }
                            }
                        }
    ...
    private void loadFromLocal(Context context, String albumName) {
        List<Song> songs = getSongsByAlbumName(context, albumName);
