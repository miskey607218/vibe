...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
     ...
     *
     * @param loginDTO 登录信息
     * @return 结果（包含 token 和 userType）
     */
    @Override
    public Result unifiedLogin(LoginDTO loginDTO) {
        String account = loginDTO.getAccount();

        // 1. 先按邮箱查找
        User user = userMapper.selectOne(new QueryWrapper<User>().eq("email", account));
        // 2. 再按用户名查找
        if (user == null) {
            user = userMapper.selectOne(new QueryWrapper<User>().eq("username", account));
        }
        ...
        if (DigestUtils.md5DigestAsHex(loginDTO.getPassword().getBytes()).equals(user.getPassword())) {
            Map<String, Object> claims = new HashMap<>();
            UserTypeEnum userType = user.getUserType() != null ? user.getUserType() : UserTypeEnum.USER;
            ...
            claims.put(JwtClaimsConstant.USERNAME, user.getUsername());
            claims.put(JwtClaimsConstant.EMAIL, user.getEmail());
            claims.put(JwtClaimsConstant.USER_TYPE, userType.getId());
            ...
            resultMap.put("userType", userType.getId()); // 0=普通用户 1=管理员

            return Result.success(MessageConstant.LOGIN + MessageConstant.SUCCESS, resultMap);
        }
    ...
    public Result updateUserPassword(UserPasswordDTO userPasswordDTO, String token) {
        ...
        updateUser.setPassword(DigestUtils.md5DigestAsHex(userPasswordDTO.getNewPassword().getBytes()));
    ...
    public Result resetUserPassword(UserResetPasswordDTO userResetPasswordDTO) {
        ...
        resetUser.setPassword(DigestUtils.md5DigestAsHex(userResetPasswordDTO.getNewPassword().getBytes()));
        resetUser.setUpdateTime(LocalDateTime.now());
        if (userMapper.update(resetUser,
    ...
    public Result addUser(UserAddDTO userAddDTO) {
        ...
        user.setIntroduction(userAddDTO.getIntroduction());
        user.setCreateTime(LocalDateTime.now());
        user.setUpdateTime(LocalDateTime.now());
