...
public class PlaylistServiceImpl extends ServiceImpl<PlaylistMapper, Playlist> implements IPlaylistService {
    ...
    public Result addSongToPlaylist(Long playlistId, Long songId) {
        ...
        Long count = playlistBindingMapper.countByPlaylistIdAndSongId(playlistId, songId);
        if (count != null && count > 0) {
            return Result.error("歌曲已在歌单中");
        }

        playlistBindingMapper.insertBinding(playlistId, songId);
    ...
    public Result removeSongFromPlaylist(Long playlistId, Long songId) {
        int rows = playlistBindingMapper.deleteByPlaylistIdAndSongId(playlistId, songId);
        if (rows == 0) {
            return Result.error("歌曲不在该歌单中");
        }
